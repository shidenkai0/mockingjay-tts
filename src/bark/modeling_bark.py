...
class BarkModel(BarkPreTrainedModel):
    ...
    def generate(
        ...
        **kwargs,
        ...
        # most_common_tokens = find_most_common_tokens(coarse_output[1])
        # print(f"most common tokens (output[1]) (among {len(most_common_tokens)} tokens): {most_common_tokens}")

        def build_mask(tensor, token_list):
            """
            Build a mask tensor of booleans based on the input tensor and token list.

            Args:
                tensor (torch.Tensor): 2D tensor containing token IDs.
                token_list (list): List of tuples containing token pairs to be masked.

            Returns:
                torch.Tensor: A mask tensor containing 1s for audio tokens that should be kept,
                0s otherwise.
            """
            num_rows, num_cols = tensor.shape
            # Initialize an empty tensor with zeros, of half the size of the input tensor
            mask = torch.ones(num_rows, num_cols // 2, dtype=torch.bool, device="cuda")

            # Convert tensor to pairs for easier comparison
            tensor_pairs = tensor.view(num_rows, -1, 2)
        ...
        mask = build_mask(coarse_output, silence_tokens + noise_tokens)

        # zip mask and coarse_output[0] viewed as pairs to visualize which tokens are masked
        # masked_tokens = list(zip(coarse_output[1].view(-1, 2).tolist(), mask[1].tolist()))
        # print(f"Masked tokens(length {len(masked_tokens)}) {masked_tokens}")

        def mask_fine_output(fine_output: torch.Tensor, mask: torch.Tensor, k: int) -> torch.Tensor:
            """
            Mask the fine output tensor based on the mask tensor.

            Args:
                fine_output (torch.Tensor): 3D tensor with shape [batch_size, num_features, sequence_length].
                mask (torch.Tensor): 2D tensor with shape [batch_size, sequence_length].
                k (int): maximum tolerated length of a sequence of zeros in the mask.
                Any sequence of zeros longer than k will be shortened to a sequence of k zeros.

            Returns:
                torch.Tensor: (1, features_dim, concatenated_and_masked_sequence_length) tensor containing the masked values.
            """
            # Check dimensions
            if fine_output.dim() != 3 or mask.dim() != 2:
                raise ValueError("The dimensions of fine_output and mask are not as expected.")

            # Check shape compatibility
            if fine_output.shape[0] != mask.shape[0] or fine_output.shape[-1] != mask.shape[-1]:
                raise ValueError("The dimensions of fine_output and mask are incompatible.")

            # Add an extra singleton dimension to mask for broadcasting
            mask_expanded = mask.unsqueeze(1)

            # Concatenate the sequences along the last dimension
            concatenated_fine_output = torch.cat(fine_output.unbind(dim=0), dim=-1)
            concatenated_mask = torch.cat(mask_expanded.unbind(dim=0), dim=-1)

            # Apply the mask
            masked_output = concatenated_fine_output * concatenated_mask

            # Flatten the mask to a 1D tensor for easier handling
            concatenated_mask_flat = concatenated_mask.squeeze()

            # Initialize a tensor to hold the modified mask values
            new_mask = torch.ones_like(concatenated_mask_flat)

            # Shorten sequences of zeros to k
            zero_indices = torch.where(concatenated_mask_flat == 0)[0]
            if len(zero_indices) == 0:
                return masked_output.unsqueeze(0)

            # Start tracking sequence of zeros
            ...
            for j in range(1, len(zero_indices)):
                if zero_indices[j] == zero_indices[j - 1] + 1:
                    ...
                    if seq_length > k:
                        # Modify mask to shorten the sequence to length k
                        remove_start = start_idx + k
                        remove_end = start_idx + seq_length
                        new_mask[remove_start:remove_end] = 0  # Fix: set to 0 instead of 1
                    # Reset start index and sequence length
                    start_idx = zero_indices[j]
                    seq_length = 1
            ...
            # Apply the new mask
            new_mask_expanded = new_mask.unsqueeze(0)
            masked_output = concatenated_fine_output * new_mask_expanded

            # Remove zeros from the masked output
            masked_output_non_zero = masked_output[:, masked_output.sum(dim=0) != 0]

            # Add singleton batch dimension
            final_output = masked_output_non_zero.unsqueeze(0)
